<img src="docs/logo-markocupic.png" width="150" alt="Logo Marko Cupic"/>
...
* Create for your backend and frontend login the `***BackendClientFactory` class (e.g. GoogleBackendClientFactory) and `***FrontendClientFactory` class (e.g. GoogleFrontendClientFactory).
  Both classes have to extend `Nistech\ContaoQualliIdClient\OAuth2\Client\AbstractClientFactory`.
...
* Create your button generator class `ButtonGenerator` that has to implement `Nistech\ContaoQualliIdClient\ButtonGenerator\ButtonGeneratorInterface`.
* If the Contao user is not identified by the claim `email`, you have to write your own token handler that has to implement `Nistech\ContaoQualliIdClient\OAuth2\Token\TokenHandlerInterface`.
